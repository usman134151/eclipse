...
class RemittanceGeneratorBooking extends Component
{
    ...
    public function dataSource($providerId)
    {
        ...
        $orphanReimbursements = BookingReimbursement::where(['provider_id' => $providerId, 'status' => 1, 'payment_status' => 0, 'remittance_id' => 0])->whereNotIn('booking_id',$bookingIds)->whereNotNull('booking_id')
        ->where('booking_id', '!=', 0)->select(['id as reimbursement_id', 'reimbursement_number', 'amount', 'booking_id'])->get()->toArray();
        ...
        return array_merge($bookings, $reimbursements,$orphanReimbursements, $payments, $invoices);
    }
