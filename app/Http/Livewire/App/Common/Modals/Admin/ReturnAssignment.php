...
class ReturnAssignment extends Component
{
    ...
    public function unassign()
    {
        ...
        if ($this->requireApproval == true) {
            ...
            if ($this->bookingProvider->provider_response != '') {
                $logMessage .= ' (Reason: ' . $this->bookingProvider->provider_response . ")";
            }
            ...
            if (!is_null($this->bookingProvider)) {
                ...
                NotificationService::sendNotification('Booking: Returned by Provider', $data,7,true);

                $this->bookingProvider->delete();
                $this->booking->status = 1;
                $this->booking->save();
                
                $message = "Provider '" . Auth::user()->name . "' surrendered booking '" . $this->booking->booking_number ."'";
