    ...
    if (fileType != "image") {
        ...
        return (
            `
                <div class="attachment-preview">
                 <span class="fas fa-times cancel"></span>
                 <div class="image-file chat-image" style="background-image: url('` + imgURL +
            ` ');"></div>
...
clientListenChannel.bind("client-contactItem", function (data) {
    if (data.to == auth_id && getMessengerId() != data.from) {
        if (data.update) {
            setMessengerId(data.from);
            if (getMessengerId() != auth_id) {
                IDinfo(data.from);
                $(".chat-container").show();
            }
...
/**
 *-------------------------------------------------------------
 * Get Total unseen Messages count
 *-------------------------------------------------------------
 */
function getTotalUnseenMessages() {
    $.ajax({
        url: url + "/" + auth_id,
        method: "GET",
        data: {_token: csrfToken, key: 'countTotalUnseen'},
        dataType: "JSON",
        success: (data) => {
            if (data == 0) {
                $("body").find(`.message-counter`).hide();
            }else {
                $("body").find(`.message-counter`).html(data);
                $("body").find(`.message-counter`).show();
            }
        },
        error: () => {
            console.error("Server error, check your response");
