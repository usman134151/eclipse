        ...
        <tbody>
            ...
            <tr class="odd js-selected-row">
                ...
                <td class="position-relative">
                    <p>
                    {{$address['address_name']}}:  {{ $address['address_line1'].' '.$address['city'].' '.$address['state'] }}<a href="#" title="Delete" aria-label="Delete" wire:click.prevent="deleteAddress({{ $loop->index }})" class="btn btn-sm btn-secondary rounded btn-hs-icon position-absolute top-0 end-0"  style="top:15px !important;right: -15px !important;" name="deleteIcon" onclick="$(this).parent().slideUp();">
