{{-- BEGIN: Assign Provider Off Canvas --}}
<x-off-canvas show="accommodationServicesAccess">
	<x-slot name="title">Accomodation & Service Access</x-slot>
    <div class="row mb-4 mt-4">
        <div class="col-md-5">
            <h4>Accommodation Name:</h4>
            <span>Spoken Language Interpreting Services</span>
        </div>
        <div class="col-md-5">
            <h4>No. of Services:</h4>
            <span>10</span>
        </div>
    </div>
	<div class="d-flex justify-content-between mb-1">
        <div class="d-inline-flex align-items-center gap-4">
            <label for="show_records_number" class="form-label">Show</label>
            <select class="form-select py-2" id="show_records_number">
                <option>10</option>
                <option>15</option>
                <option>20</option>
                <option>25</option>
            </select>
        </div>
        <div class="d-inline-flex align-items-center gap-4">
            <label for="search" class="form-label fw-semibold">Search</label>
            <input type="search" class="form-control py-2" id="search" name="search" placeholder="Search here" autocomplete="on"/>
        </div>
    </div>
	<div class="table-responsive">
		<table id="" class="table table-hover" aria-label="Accomodation and Service Access Table">
            <thead>
                <tr role="row">
                    <th scope="col" class="text-center">
                        <input class="form-check-input" type="checkbox" value="" aria-label="Select All services">
                    </th>
                    <th scope="col">Service</th>
                    <th scope="col"> Permission</th>
                    <th scope="col">Actions</th>
                </tr>
            </thead>
            <tbody>
                <tr role="row" class="odd">
                    <td class="text-center">
                    <input class="form-check-input" type="checkbox" value="" aria-label="Select">
                    </td>
                    <td>
                     <p>Coaching Service</p>
                    </td>
                    <td>
                        <div class="form-check form-switch">
                            <input class="form-check-input" type="checkbox" role="switch" id="permissionsToggle" checked aria-label="Permissions Toggle">
                            <label class="form-check-label" for="permissionsToggle">Visible</label>
                            <label class="form-check-label" for="permissionsToggle">Manage</label>
                        </div>
                    </td>
                    <td>
                        <div class="d-flex actions">
                            <a href="javascript:void(0)" title="Delete Team" aria-label="Delete" class="btn btn-sm btn-secondary rounded btn-hs-icon">
                                <x-icon name="recycle-bin"/>
                            </a>
                        </div>
                    </td>
                </tr>
                <tr role="row" class="odd">
                    <td class="text-center">
                    <input class="form-check-input" type="checkbox" value="" aria-label="Select">
                    </td>
                    <td>
                     <p>Copy of English to Japanese Interpreting</p>
                    </td>
                    <td>
                        <div class="form-check form-switch">
                            <input class="form-check-input" type="checkbox" role="switch" id="permissionsToggle" aria-label="Permissions Toggle">
                            <label class="form-check-label" for="permissionsToggle">Visible</label>
                            <label class="form-check-label" for="permissionsToggle">Manage</label>
                        </div>
                    </td>
                    <td>
                        <div class="d-flex actions">
                            <a href="javascript:void(0)" title="Delete Team" aria-label="Delete" class="btn btn-sm btn-secondary rounded btn-hs-icon">
                                <x-icon name="recycle-bin"/>
                            </a>
                        </div>
                    </td>
                </tr>
                <tr role="row" class="odd">
                    <td class="text-center">
                    <input class="form-check-input" type="checkbox" value="" aria-label="Select">
                    </td>
                    <td>
                     <p>Copy of English to Portuguese Interpreting</p>
                    </td>
                    <td>
                        <div class="form-check form-switch">
                            <input class="form-check-input" type="checkbox" role="switch" id="permissionsToggle" aria-label="Permissions Toggle">
                            <label class="form-check-label" for="permissionsToggle">Visible</label>
                            <label class="form-check-label" for="permissionsToggle">Manage</label>
                        </div>
                    </td>
                    <td>
                        <div class="d-flex actions">
                            <a href="javascript:void(0)" title="Delete Team" aria-label="Delete" class="btn btn-sm btn-secondary rounded btn-hs-icon">
                                <x-icon name="recycle-bin"/>
                            </a>
                        </div>
                    </td>
                </tr>
                <tr role="row" class="odd">
                    <td class="text-center">
                    <input class="form-check-input" type="checkbox" value="" aria-label="Select">
                    </td>
                    <td>
                     <p>English to French Interpreting</p>
                    </td>
                    <td>
                        <div class="form-check form-switch">
                            <input class="form-check-input" type="checkbox" role="switch" id="permissionsToggle" checked aria-label="Permissions Toggle">
                            <label class="form-check-label" for="permissionsToggle">Visible</label>
                            <label class="form-check-label" for="permissionsToggle">Manage</label>
                        </div>
                    </td>
                    <td>
                        <div class="d-flex actions">
                            <a href="javascript:void(0)" title="Delete Team" aria-label="Delete" class="btn btn-sm btn-secondary rounded btn-hs-icon">
                                <x-icon name="recycle-bin"/>
                            </a>
                        </div>
                    </td>
                </tr>
                <tr role="row" class="odd">
                    <td class="text-center">
                    <input class="form-check-input" type="checkbox" value="" aria-label="Select">
                    </td>
                    <td>
                     <p>English to Japanese Interpreting</p>
                    </td>
                    <td>
                        <div class="form-check form-switch">
                            <input class="form-check-input" type="checkbox" role="switch" id="permissionsToggle" aria-label="Permissions Toggle">
                            <label class="form-check-label" for="permissionsToggle">Visible</label>
                            <label class="form-check-label" for="permissionsToggle">Manage</label>
                        </div>
                    </td>
                    <td>
                        <div class="d-flex actions">
                            <a href="javascript:void(0)" title="Delete Team" aria-label="Delete" class="btn btn-sm btn-secondary rounded btn-hs-icon">
                                <x-icon name="recycle-bin"/>
                            </a>
                        </div>
                    </td>
                </tr>
                <tr role="row" class="odd">
                    <td class="text-center">
                    <input class="form-check-input" type="checkbox" value="" aria-label="Select">
                    </td>
                    <td>
                     <p>English to Spanish Interpreting</p>
                    </td>
                    <td>
                        <div class="form-check form-switch">
                            <input class="form-check-input" type="checkbox" role="switch" id="permissionsToggle" checked aria-label="Permissions Toggle">
                            <label class="form-check-label" for="permissionsToggle">Visible</label>
                            <label class="form-check-label" for="permissionsToggle">Manage</label>
                        </div>
                    </td>
                    <td>
                        <div class="d-flex actions">
                            <a href="javascript:void(0)" title="Delete Team" aria-label="Delete" class="btn btn-sm btn-secondary rounded btn-hs-icon">
                                <x-icon name="recycle-bin"/>
                            </a>
                        </div>
                    </td>
                </tr>
                <tr role="row" class="odd">
                    <td class="text-center">
                    <input class="form-check-input" type="checkbox" value="" aria-label="Select">
                    </td>
                    <td>
                     <p>English to German Interpreting</p>
                    </td>
                    <td>
                        <div class="form-check form-switch">
                            <input class="form-check-input" type="checkbox" role="switch" id="permissionsToggle" checked aria-label="Permissions Toggle">
                            <label class="form-check-label" for="permissionsToggle">Visible</label>
                            <label class="form-check-label" for="permissionsToggle">Manage</label>
                        </div>
                    </td>
                    <td>
                        <div class="d-flex actions">
                            <a href="javascript:void(0)" title="Delete Team" aria-label="Delete" class="btn btn-sm btn-secondary rounded btn-hs-icon">
                                <x-icon name="recycle-bin"/>
                            </a>
                        </div>
                    </td>
                </tr>               
            </tbody>
        </table>
	  </div>
</x-off-canvas>
{{-- END: Admin Staff Team Details Off Canvas --}}